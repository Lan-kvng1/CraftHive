// app/(customer)/language.tsx — FINAL
// Language picker — selection persists and changes all text immediately
...
export default function CustomerLanguage() {
  ...
  const onSelect = (code: Language) => {
    setLanguage(code)
    // Give feedback that language changed
  ...
  const s = StyleSheet.create({
    ...
    infoCard: { backgroundColor: C.card, marginHorizontal: 16, marginTop: 16, borderRadius: 16, padding: 14, flexDirection: 'row', alignItems: 'flex-start', gap: 12 },
    ...
    ghanaNote: { fontSize: 12, color: C.textMuted, marginHorizontal: 16, marginBottom: 4, fontStyle: 'italic' },
  ...
  const LangRow = ({ item, last = false }: { item: (typeof LANGUAGE_OPTIONS)[0]; last?: boolean }) => (
    <TouchableOpacity
      style={[s.langRow, last && s.langRowLast, language === item.code && s.activeRow]}
      onPress={() => onSelect(item.code as Language)}
      activeOpacity={0.7}>
      <Text style={s.flagTxt}>{item.flag}</Text>
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <ScrollView style={s.body} showsVerticalScrollIndicator={false}>
        ...
        <Text style={s.ghanaNote}>{t('language.ghanaNote')}</Text>
        <View style={s.section}>
          {ghanaLangs.map((l, i) => (
            <LangRow key={l.code} item={l} last={i === ghanaLangs.length - 1} />
          ))}
        </View>
        ...
        <View style={s.section}>
          {otherLangs.map((l, i) => (
            <LangRow key={l.code} item={l} last={i === otherLangs.length - 1} />
          ))}
        </View>

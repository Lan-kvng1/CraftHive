// app/(artisan)/chat.tsx — WhatsApp style
...
export default function ArtisanChat() {
    ...
    const [previewUri, setPreviewUri] = useState<string | null>(null)  // recorded but not yet sent
    ...
    const load = useCallback(async () => {
        ...
        const { data: b } = await supabase.from('bookings')
            .select('id, title, service_type, status, customer_id').eq('id', bookingId).single()
        setBooking(b)
        if (b?.customer_id) {
            const { data: cp } = await supabase.from('profiles').select('id, full_name, avatar_url, phone').eq('id', b.customer_id).single()
            setArtisan(cp)
        }
        const { data: msgs } = await supabase.from('messages')
            .select('*').eq('booking_id', bookingId).eq('is_deleted', false).order('created_at', { ascending: true })
        setMessages(msgs || [])
        setLoading(false)
    ...
    // Update own last_seen every 30s + on mount
    useEffect(() => {
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'messages', filter: `booking_id=eq.${bookingId}` },
                async (payload: any) => {
                    if (payload.eventType === 'INSERT') {
                        // Fetch full message from DB — Realtime postgres_changes
                        // does not include jsonb columns in payload.new, so audio/
                        // image/location messages would show as plain text if we
                        // used payload.new directly.
                        ...
                        setMessages(prev => {
                            ...
                            if (without.find(m => m.id === msg.id)) return without
                            return [...without, msg]
                        })
    ...
    const send = async (
        ...
        if (!booking?.customer_id || sending) return
        setSending(true)
        setText('')
        setShowAttach(false)
        ...
        const tempMsg = {
            ...
            sender_id: user?.id, receiver_id: booking.customer_id,
    ...
    const commitEdit = async () => {
        ...
        // We will store { edited: true } in payload to show the 'edited' badge
    ...
    // ── Playback ──────────────────────────────────────────────
    // Reset audio session — fixes -11850 AVFoundationErrorDomain on iOS
    // expo-audio handles session automatically — no manual reset needed
    ...
    const statusColor: Record<string, string> = { pending: '#F5A623', confirmed: NAVY, in_progress: '#8B5CF6', completed: GREEN }
    ...
    const s = StyleSheet.create({
        ...
        statusPill: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 5 },
        statusDot: { width: 7, height: 7, borderRadius: 4 },
        statusTxt: { fontSize: 11, color: '#FFF', fontWeight: '600', textTransform: 'capitalize' },
    ...
    return (
        <SafeAreaView style={s.safe} edges={['top']}>
            ...
            <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                ...
                {/* Recording waveform strip — appears above input bar while recording */}
                {isRecording && (
                    ...
                    <View style={s.previewBar}>
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={{ fontSize: 13, color: C.textSecondary, fontWeight: '600' }}>
                                Voice message · {fmtDur(previewDuration)}
                            </Text>
                            <Text style={{ fontSize: 11, color: C.textMuted }}>
                                {previewPlaying ? 'Playing...' : 'Tap ▶ to preview before sending'}
                            </Text>
                        </View>
                ...
                {/* Input bar — always visible, mic turns red while recording */}
                {!previewUri && (
            ...
            <Modal visible={!!imageViewUrl} transparent animationType="fade">
                <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.95)', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    {imageViewUrl && (
                        <Image source={{ uri: imageViewUrl }} style={{ width: '100%', height: '80%' }} resizeMode="contain" />
                    )}
                </View>
            </Modal>

            {/* In-app WebView for documents and location */}

// app/(artisan)/settings.tsx — FINAL
// Navy, useAppTheme, AudioContext, no duplicates with profile
...
export default function ArtisanSettings() {
    ...
    const notifPayments = prefs.payments
    ...
    const handleLogout = () => Alert.alert(t('common.logout'), 'Are you sure?', [
    ...
    const handleDeleteAccount = () => Alert.alert(t('settings.deleteAccount'), 'This will permanently delete your account.', [
        { text: t('common.cancel'), style: 'cancel' },
        { text: t('common.delete'), style: 'destructive', onPress: () => Alert.alert('Request Submitted', 'Our team will process this within 7 days.') },
    ])

    const changePassword = async () => {
        if (!pwNew.trim()) { Alert.alert(t('common.required'), 'Enter a new password.'); return }
        if (pwNew.length < 6) { Alert.alert(t('common.error'), 'At least 6 characters.'); return }
        ...
        Alert.alert(t('common.success'), 'Password updated.')
    }
    ...
    const s = StyleSheet.create({
        ...
        sectionLabel: { fontSize: 11, fontWeight: '800', color: C.textMuted, letterSpacing: 1.2, textTransform: 'uppercase', marginHorizontal: 16, marginTop: 24, marginBottom: 8 },
        section: { backgroundColor: C.card, marginHorizontal: 16, borderRadius: 18, overflow: 'hidden' },
        row: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: C.border, gap: 14 },
            ...
            <ScrollView style={s.body} showsVerticalScrollIndicator={false}
                ...
                <View style={s.section}>
                    <View style={s.row}>
                        ...
                        <Text style={s.rowLabel}>{t('settings.bookingAlerts')}</Text>
                    ...
                    <View style={[s.row, s.rowLast]}>
                        ...
                        <Text style={s.rowLabel}>{t('settings.paymentReceived')}</Text>
                        <Switch value={notifPayments} onValueChange={(v) => setPrefs({ payments: v })} trackColor={{ true: NAVY }} thumbColor="#FFF" />
                    </View>
                </View>

                <Text style={s.sectionLabel}>{t('settings.preferences')}</Text>
                <View style={s.section}>
                    <Row icon={SettingsIcon} label={t('settings.theme')}
                        value={THEMES.find(th => th.key === mode)?.label || t('settings.themeSystem')}
                        onPress={() => setThemeVisible(true)} />
                    <Row icon={GlobeIcon} label={t('settings.language')}
                        value={currentLangLabel}
                        onPress={() => router.push('/(artisan)/language' as any)} />
                    <View style={[s.row, s.rowLast]}>
                        <View style={s.rowIcon}><VolumeIcon size={17} color={NAVY} /></View>
                        <View style={{ flex: 1 }}><Text style={s.rowLabel}>{t('settings.audioReader')}</Text></View>
                        <Switch value={audioEnabled} onValueChange={setAudioEnabled} trackColor={{ true: NAVY, false: '#CBD5E1' }} thumbColor="#FFF" />
                    </View>
                </View>
                {audioEnabled && (
                ...
                <View style={s.section}>
                    <View style={[s.row, s.rowLast]}>
                        ...
                        <View style={{ flex: 1 }}><Text style={s.rowLabel}>{t('settings.lowDataMode')}</Text></View>
                        <Switch value={lowDataMode} onValueChange={setLowDataMode} trackColor={{ true: NAVY, false: '#CBD5E1' }} thumbColor="#FFF" />
                    </View>
                </View>
                {lowDataMode && (
                ...
                <View style={s.section}>
                    <Row icon={ShieldIcon} label={t('settings.helpSupport')} onPress={() => router.push('/(artisan)/help-support' as any)} />
